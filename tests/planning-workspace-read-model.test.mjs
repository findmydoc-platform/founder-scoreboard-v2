...
test("Planning and Strategic Planning use consumer-owned loaders and no legacy workspace scope", async () => {
  ...
  assert.doesNotMatch(projectsReader, /server\//);
  assert.match(planningClient, /requestPlanningWorkspaceData/);
  assert.doesNotMatch(projectsClient, /features\/planning\/server|features\/planning-items\/server/);
});
