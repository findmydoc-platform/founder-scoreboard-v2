...
test("fmd tools workspace keeps internal tools repos notion and drive visible", async () => {
  ...
  const toolRow = await readFile("src/features/tools/molecules/fmd-tool-registry-row.tsx", "utf8");
  ...
  const toolSeedExamplesMigration = await readFile("supabase/0040_werkzeugraum_seed_examples.sql", "utf8");
  ...
  assert.match(toolsSurface, /Tool eintragen/);
  assert.match(toolsSurface, /role="dialog"/);
  assert.match(toolsSurface, /Tool bearbeiten/);
  ...
  assert.match(toolRow, /title="Tool bearbeiten"/);
  ...
  assert.match(toolCommands, /new URL\(url\)/);
