...
test("repo readiness includes the GitHub Actions deployment pipeline gates", async () => {
  ...
  assert.match(productionWorkflow, /pnpm run deploy:supabase-schema[\s\S]*pnpm run verify:supabase[\s\S]*vercel-deploy-prebuilt\.sh production/);
  ...
  assert.match(pkg, /deploy:supabase-schema/);
  assert.match(pkg, /pnpm test && pnpm run verify:vercel-ready/);
