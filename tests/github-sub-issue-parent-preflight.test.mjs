...
for (const scenario of [
  ...
  {
    name: "not linked",
    data: validParent({ github_issue_number: null, github_issue_url: "" }),
    error: null,
    message: /noch nicht mit GitHub verknüpft/,
  },
]) {
...
test("rejects contradictory local parent references before reaching GitHub", async () => {
  let githubReads = 0;
  const preflight = await loadPreflight(async () => {
    githubReads += 1;
    return {
  ...
  await assert.rejects(
    () => preflight.preflightGitHubSubIssueParent(
      supabaseWithParent(validParent({ github_issue_url: "https://github.com/findmydoc-platform/management/issues/43" })),
      { parentTaskId: "parent-1" },
      "installation-token",
    ),
    /lokale GitHub-Verknüpfung.*widersprüchlich/,
  );
  assert.equal(githubReads, 0);
});

test("rejects a pull request parent before the child write", async () => {
  const preflight = await loadPreflight(async () => ({
    number: 42,
  ...
  await assert.rejects(
    ...
    /stimmt nicht mit der lokalen Verknüpfung überein/,
  );
});
