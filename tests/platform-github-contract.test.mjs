...
test("task relationships use github-like blocked by and blocking semantics", async () => {
  ...
  assert.match(relationshipHook, /method: "POST"/);
  assert.match(relationshipHook, /method: "DELETE"/);
...
test("github oauth prepares user-based sync without storing provider tokens", async () => {
  ...
  assert.match(commentHook, /x-github-provider-token/);
  assert.match(commentHook, /getRememberedGitHubProviderToken/);
  assert.match(commentComposer, /onUploadAttachment/);
  assert.match(githubCommentImage, /getRememberedGitHubProviderToken/);
