...
import {
  cleanText as clean,
  compactText as trimText,
  ensurePeriod,
  firstSentence,
  hasQuestionMarkReplacement,
...
for (const { task, patch } of patches) {
  for (const [field, value] of Object.entries(patch)) {
    if (hasQuestionMarkReplacement(value)) suspicious.push({ id: task.id, field, value });
  }
}
...
if (!APPLY) {
  ...
  const backupPath = taskTemplateBackupPath("task-template-v2-backfill-backup");
