...
is 'Atomically assigns up to 100 eligible Deliverables to one unlocked Sprint with compare-and-set protection, authoritative actor validation, and per-item audit history.';

revoke all on function public.assign_backlog_tasks_to_sprint_transaction(jsonb, text, text, text, text) from public, anon, authenticated;
