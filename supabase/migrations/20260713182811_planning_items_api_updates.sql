-- Planning Items API: public contract is new; existing token storage stays in place
-- so existing token prefixes and credential-store identifiers remain valid.
...
FOR EACH ROW
EXECUTE FUNCTION public.touch_package_updated_at();
...
CREATE TABLE IF NOT EXISTS public.team_planning_item_update_requests (
  ...
  item_type text NOT NULL CHECK (item_type IN ('initiative', 'deliverable', 'sub_issue')),
...
CREATE OR REPLACE FUNCTION public.create_team_planning_items_token(
  p_profile_id text,
  p_label text,
  p_token_hash text,
  p_token_hint text,
...
CREATE OR REPLACE FUNCTION public.revoke_team_planning_items_token(
  p_token_id uuid,
  p_profile_id text
) RETURNS uuid
LANGUAGE plpgsql
...
begin
  ...
  if p_item_type = 'initiative' then
  ...
  if exists (select 1 from jsonb_object_keys(coalesce(p_patch, '{}'::jsonb))) then
    insert into public.audit_log (
      ...
      p_profile_id, 'team.planning_items.update', p_item_type, p_item_id, v_before, v_item, p_request_ip, p_user_agent
    );
  end if;
...
REVOKE ALL ON FUNCTION public.authenticate_team_planning_items_token(text, text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.authenticate_team_planning_items_token(text, text) TO service_role;

REVOKE ALL ON FUNCTION public.create_team_planning_items_token(text, text, text, text, boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION public.create_team_planning_items_token(text, text, text, text, boolean) TO service_role;

REVOKE ALL ON FUNCTION public.revoke_team_planning_items_token(uuid, text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.revoke_team_planning_items_token(uuid, text) TO service_role;

REVOKE ALL ON FUNCTION public.create_team_planning_items_transaction(uuid, text, uuid, text, jsonb, text, text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.create_team_planning_items_transaction(uuid, text, uuid, text, jsonb, text, text) TO service_role;

REVOKE ALL ON FUNCTION public.update_team_planning_item_transaction(uuid, text, text, text, timestamptz, uuid, text, jsonb, jsonb, jsonb, text, text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.update_team_planning_item_transaction(uuid, text, text, text, timestamptz, uuid, text, jsonb, jsonb, jsonb, text, text) TO service_role;

REVOKE ALL ON TABLE public.team_planning_item_update_requests FROM PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.team_planning_item_update_requests TO service_role;
