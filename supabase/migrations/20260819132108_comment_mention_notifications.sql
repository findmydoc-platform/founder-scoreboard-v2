...
alter table public.task_external_comments
  add column if not exists source_updated_at timestamptz,
  add column if not exists mention_recipient_profile_ids text[] not null default '{}',
  add column if not exists mention_recipients_initialized boolean not null default false;

alter table public.tasks
  add column if not exists github_comment_notifications_after timestamptz;

update public.task_external_comments
...
comment on column public.notification_events.actor_label is
  'Trusted display label for an external actor that has no FounderOps profile.';
comment on column public.notification_events.target_path is
  'Optional internal relative path for opening the exact notification target.';
comment on column public.task_external_comments.source_updated_at is
  'Last update timestamp reported by the external source.';
comment on column public.task_external_comments.mention_recipient_profile_ids is
  'FounderOps profile IDs resolved from the last accepted external comment snapshot.';
comment on column public.task_external_comments.mention_recipients_initialized is
  'Whether mention_recipient_profile_ids reflects an accepted post-migration snapshot.';
comment on column public.tasks.github_comment_notifications_after is
  'GitHub comments updated before this task-specific watermark must not create mention notifications.';

create or replace function public.set_github_comment_notification_watermark()
returns trigger
language plpgsql
...
drop trigger if exists tasks_github_comment_notification_watermark on public.tasks;
create trigger tasks_github_comment_notification_watermark
before insert or update of task_type, github_repo, github_issue_number, issue_number, github_issue_url, issue_url
on public.tasks
for each row execute function public.set_github_comment_notification_watermark();

-- verify:watermark-backfill:start
with valid_tasks as (
  select task.id
...
set github_comment_notifications_after = clock_timestamp()
from valid_tasks
where task.id = valid_tasks.id
  and task.github_comment_notifications_after is null;
-- verify:watermark-backfill:end
...
begin
  ...
  for v_comment in select value from jsonb_array_elements(coalesce(p_comments, '[]'::jsonb)) loop
    ...
    then
      ...
      where recipient_id is not null
        and recipient_id is distinct from v_actor_profile_id
...
begin
  ...
  if p_operation = 'upsert' then
    ...
    then
      ...
      from unnest(v_current_recipient_profile_ids) recipient_id
      where nullif(trim(recipient_id), '') is not null
        and recipient_id is distinct from nullif(trim(coalesce(p_actor_profile_id, '')), '')
        and not (recipient_id = any(v_previous_recipient_profile_ids))
      on conflict (dedupe_key) where dedupe_key is not null do nothing;
    end if;
  else
...
revoke all on function public.create_task_comment_with_notifications(text, text, text, boolean, text[], text[])
  from public, anon, authenticated;
revoke all on function public.set_github_comment_notification_watermark()
  from public, anon, authenticated;
revoke all on function public.import_github_task_comments_with_mentions(text, jsonb)
  from public, anon, authenticated;
revoke all on function public.apply_github_issue_comment_webhook_projection_with_mentions(text, uuid, text, text, timestamptz, text, text, text, text, timestamptz, timestamptz, text, text[], text[], timestamptz)
  from public, anon, authenticated;

grant execute on function public.create_task_comment_with_notifications(text, text, text, boolean, text[], text[])
  to service_role;
grant execute on function public.import_github_task_comments_with_mentions(text, jsonb)
  to service_role;
grant execute on function public.apply_github_issue_comment_webhook_projection_with_mentions(text, uuid, text, text, timestamptz, text, text, text, text, timestamptz, timestamptz, text, text[], text[], timestamptz)
  to service_role;
