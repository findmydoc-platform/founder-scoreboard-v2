...
export async function handleBrowserMilestoneUpdate(request: NextRequest, routeContext: MilestoneRouteContext) {
  ...
  if (!actor.ok) return apiError("Meilenstein konnte nicht gespeichert werden.", 403);
...
export async function handleBrowserMilestoneDelete(request: NextRequest, routeContext: MilestoneRouteContext) {
  const context = await requireJsonApiContext<unknown>(request, requireOperationalLead, null);
  if (!context.ok) return context.response;
