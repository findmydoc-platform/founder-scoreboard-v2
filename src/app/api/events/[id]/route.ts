...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isFinite(id)) return NextResponse.json({ error: "Event ist erforderlich." }, { status: 400 });
  ...
  if (currentError || !current) return NextResponse.json({ error: "Event wurde nicht gefunden." }, { status: 404 });
  ...
  if (error || !event) return NextResponse.json({ error: error?.message || "Event konnte nicht aktualisiert werden." }, { status: 500 });
