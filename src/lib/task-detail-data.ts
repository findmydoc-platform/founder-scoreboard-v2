...
export async function loadTaskDetailData(supabase: SupabaseClient, taskId: string): Promise<TaskDetailDataResult> {
  ...
  const [
    ...
    supabase.from("task_comments").select("id,task_id,profile_id,comment,created_at,github_delivery_applicable,task_comment_github_deliveries(status,github_comment_url)").eq("task_id", taskId).order("created_at", { ascending: false }).limit(200),
