  ...
  return (
    <div className="grid gap-4">
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          {[
            ...
            <button
              ...
              className={`h-8 rounded-md border px-3 text-xs font-semibold ${
                filter === option.id ? "border-blue-300 bg-blue-50 text-blue-700" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
              }`}
            >
              {option.label}
            </button>
        ...
        {message && <p className="mt-3 rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-800">{message}</p>}
      </section>
      ...
      {formOpen && canManageEvents && (
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            <label className="grid gap-1 text-xs font-semibold text-slate-500">
              ...
              <input
                ...
                className="h-10 rounded-md border border-slate-200 px-3 text-sm font-normal text-slate-900 outline-none focus:border-blue-400"
            ...
            <label className="grid gap-1 text-xs font-semibold text-slate-500 lg:col-span-2">
              ...
              <textarea
                ...
                className="min-h-24 rounded-md border border-slate-200 px-3 py-2 text-sm font-normal text-slate-900 outline-none focus:border-blue-400"
          ...
          <div className="mt-4 flex flex-wrap justify-end gap-2">
            <button type="button" onClick={closeForm} className="h-9 rounded-md border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-600 hover:bg-slate-50">
              Abbrechen
            </button>
            <button type="button" onClick={submit} disabled={pending || !draft.title.trim() || !draft.startsAt || (draft.audienceMode === "selected" && !draft.participantProfileIds.length)} className="h-9 rounded-md bg-blue-600 px-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full border border-blue-100 bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">{founderEventCategoryLabel(event.category)}</span>
                  <span className={`rounded-full border px-2 py-1 text-xs font-semibold ${event.status === "cancelled" ? "border-rose-100 bg-rose-50 text-rose-700" : event.status === "done" ? "border-emerald-100 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-50 text-slate-600"}`}>
                    {founderEventStatusLabel(event.status)}
                  </span>
                  {event.reminderGeneratedAt && <span className="rounded-full border border-emerald-100 bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700">Reminder erzeugt</span>}
                </div>
              ...
              {canManageEvents && (
                <button type="button" onClick={() => openEdit(event)} className="inline-flex h-8 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 hover:bg-slate-50">
                  <Pencil size={14} />
                  Bearbeiten
                </button>
              )}
