...
description: Use only when publishing this repository's FounderOps GitHub releases, computing the next release tag, drafting management-focused release notes from merged pull requests, or sending release announcements to Google Chat through GitHub Actions.
---
...
2. Run `node .agents/skills/release-publish/scripts/publish-release.mjs --dry-run` first.
3. Review the PR control list, release notes draft, and proposed German Google Chat message.
4. Publish with `node .agents/skills/release-publish/scripts/publish-release.mjs --execute`.
5. Send the approved message with `node .agents/skills/release-publish/scripts/send-google-chat-message.mjs --release-tag <tag> --message-file <path> --yes`.
