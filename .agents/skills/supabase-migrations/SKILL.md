...
6. Decide whether the migration needs `tests/migrations/<migration-name>.test.mjs`. Add one only for data transformations, deletion risk, non-additive structural changes, or authorization boundaries. Ordinary additive tables, columns, indexes, comments, and equivalent low-risk changes do not require a migration-specific test.
7. For a required migration test, reset the real local database to the previous migration version, insert representative existing rows, apply the target migration, and prove that primary keys, relevant values, and row counts survive unless the migration deliberately transforms them. For RLS, grants, or Auth changes, cover allowed and denied access with mapped and unmapped sessions and every affected app role. Do not inspect SQL source text or export production internals only for tests.
...
- Before changing the baseline, list every migration-specific test that the squash would supersede and explicitly tell the user that those tests should be removed with their migrations. Move any durable RLS, permission, or data-integrity rule into a normal integration test before removal.
- Remove superseded migrations and their migration-specific tests together in the same reviewed squash change. Historical backfills do not belong in the schema-only baseline; required storage bootstrap data must remain idempotent.
