...
- Scope: Phase 4.4 code-quality refactor for `src/components/task-detail-page.tsx`.
- Changes: Extracted task relationship state, optimistic add/remove behavior, and `/api/tasks/[id]/relationships` client calls into `src/hooks/use-task-relationships.ts`.
- Files: `src/components/task-detail-page.tsx`, `src/hooks/use-task-relationships.ts`, `tests/platform-contract.test.mjs`.
...
- Scope: Phase 4.5 code-quality refactor for `src/components/task-detail-page.tsx`.
- Changes: Extracted local comment state, attachment upload, GitHub comment import, auto-import, and task activity append behavior into `src/hooks/use-task-comments.ts`.
- Files: `src/components/task-detail-page.tsx`, `src/hooks/use-task-comments.ts`, `tests/platform-contract.test.mjs`.
...
- Scope: Phase 4.6 code-quality refactor for `src/components/execution-layer-overview.tsx`.
- Changes: Extracted focus metrics, owner scoping, hygiene alert filtering, suggested-task ordering, profile visibility, and shared labels/colors into `src/lib/execution-layer-view-model.ts`.
- Files: `src/components/execution-layer-overview.tsx`, `src/lib/execution-layer-view-model.ts`, `tests/platform-contract.test.mjs`.
...
- Scope: Phase 4.7 code-quality refactor for `src/components/meeting-finder-overview.tsx`.
- Changes: Extracted Meeting Finder derived data into `src/lib/meeting-finder-view-model.ts` and availability form/dialog state plus local create/edit helpers into `src/hooks/use-meeting-availability-editor.ts`.
- Files: `src/components/meeting-finder-overview.tsx`, `src/lib/meeting-finder-view-model.ts`, `src/hooks/use-meeting-availability-editor.ts`, `tests/platform-contract.test.mjs`.
- Follow-up: `meeting-finder-overview.tsx` is now below the 500-line hotspot threshold; next Phase 4 target is `src/components/sprint-score-overview.tsx`.

## 2026-06-11 - Sprint Score View Model And Meeting Attendance Section Refactor

- Scope: Phase 4.8 code-quality refactor for `src/components/sprint-score-overview.tsx`.
- Changes: Extracted sprint selection, score rows, review task lists, meeting lookup, score counters, and checklist helpers into `src/lib/sprint-score-view-model.ts`; extracted Biweekly meeting attendance table into `src/components/sprint-meeting-attendance-section.tsx`.
- Files: `src/components/sprint-score-overview.tsx`, `src/lib/sprint-score-view-model.ts`, `src/components/sprint-meeting-attendance-section.tsx`, `tests/platform-contract.test.mjs`.
...
- Follow-up: `src/lib/planning-data.ts` is now below the 500-line hotspot threshold. Remaining Phase 4 hotspots are `src/components/planning-app.tsx` and `tests/platform-contract.test.mjs`; `planning-app.tsx` needs a Level 4 plan before any large deletion or legacy cleanup.
...
- Follow-up: The contract test hotspot is gone; the only remaining stewardship hotspot is `src/components/planning-app.tsx`, which should be handled with a Level 4 baseline before large legacy cleanup or deletion.

## 2026-06-11 - Planning App Legacy Sprint Component Removal

- Scope: Phase 4.11 Level 4 code-reduction checkpoint for `src/components/planning-app.tsx`.
- Evidence: `SprintScoreOverview` and `SprintScoreOverviewLegacy` were referenced only by their local definitions and final `void` statements; the active Sprint workspace renders imported `SprintScoreTableOverview` from `src/components/sprint-score-overview.tsx`, with review helpers in `src/lib/sprint-score-view-model.ts`.
...
- Files: `src/components/planning-app.tsx`.
...
- Scope: Phase 4.12 Level 3 refactor for `src/components/planning-app.tsx`.
- Changes: Extracted auth/session state, GitHub OAuth sign-in/out, provider-token keepalive, protected planning-data loading, and protected data cache handling into `src/hooks/use-planning-auth.ts`.
- Files: `src/components/planning-app.tsx`, `src/hooks/use-planning-auth.ts`, `tests/platform-github-contract.test.mjs`, `tests/platform-planning-workflow-contract.test.mjs`.
...
- Scope: Phase 4.13 Level 3 refactor for `src/components/planning-app.tsx`.
- Changes: Extracted workspace URL restoration, local workspace persistence, and URL normalization into `src/hooks/use-planning-workspace.ts`.
- Files: `src/components/planning-app.tsx`, `src/hooks/use-planning-workspace.ts`, `tests/platform-operations-contract.test.mjs`.
...
- Scope: Phase 4.14 Level 3 refactor for `src/components/planning-app.tsx`.
